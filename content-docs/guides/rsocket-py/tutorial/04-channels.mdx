...
There is a simpler method for doing this. By passing a `payload_mapper` argument to the RequestRouter and specifying a
...
In the `shared` module add the following helper method:
...
This method converts the payload data to some type hints. It assumes that the payload data is either:
...
Once this is done, the signature of the routed methods may be simplified. For example the `join_channel` method:
...
The `send_message` method can be simplified to:
