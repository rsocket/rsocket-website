...
The <b>SessionId</b> defined in *Lines 5-6* is required in order to store the string session-id as a [weak reference](https://docs.python.org/3/library/weakref.html) later on.
...
*Lines 13-15* define the <b>ChatData</b> dataclass which represents the application's data. For now, it contains only a dict for looking up
