...
All of the above methods receive a single argument, a string representing the route.
...
- Any argument named *composite_metadata* (regardless of type-hint) will receive a `CompositeMetadata` instance containing parsed composite metadata
- Any other argument without a type-hint will receive the `Payload`

The `RequestRouter` has additional functionality which will be covered in later sections of the guide.
