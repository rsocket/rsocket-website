...
*Lines 9-12* define a request from the client which specified which statistics it wants and how often to report. The <b>ids</b> list
represents the two values in the <b>ServerStatistics</b> class.
...
First we will add a field on the <b>UserSessionData</b> to store the last statistics sent by the client.
...
We will add the endpoints for receiving/sending statistics from/to the client.
...
*Lines 16-19* defines an endpoint for receiving statistics from the client. It uses the fire-and-forget request type, since this
data is not critical to the application.

*Lines 21-60* defines an endpoint for sending statistics to the client. It uses the request-channel request type, which will allow
...
The <b>send_statistics</b> uses a fire-and-forget request (*Line 15*) to send statistics to the server. This request does not receive a response,
...
Next we will request statistics from the server:
...
from .shared import ServerStatistics
