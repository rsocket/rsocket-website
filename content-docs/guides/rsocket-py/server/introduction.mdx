...
To get started creating an RSocket server, you will first need to install the [`rsocket-core`](https://www.npmjs.com/package/rsocket-core) NPM package, and atleast one transport protocol implementation. See the server portion of [WebSocket Client Server Example](/guides/rsocket-py#server-example) for an example of an implemented `getRequestHandler`.
